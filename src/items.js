...
const Item = function ItemConstructor(config) {
  ...
  self.init = config => {
    ...
    if (self !== null) {
      Data.loadListeners(config, objects_scripts_dir, Data.loadBehaviors(config, 'objects/', self));
    }
  };
   ...
   * @return {string}
   */
  self.getShortDesc = () => self.short_description;

  /**
   * @return {string}
   */
  self.getRoomDesc = () => self.room_description || self.getShortDesc();
