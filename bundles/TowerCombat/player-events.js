    ...
    damaged: (state) =>
      function (damage, finalAmount) {
        ...
        if (damage.source !== damage.attacker) {
          buf +=
            (damage.attacker ? "'s " : " ") + `<b>${damage.source.name}</b>`;
...
function promptBuilder(promptee) {
  ...
  const getHealthPercentage = (entity) =>
    Math.floor(
      (entity.getAttribute("health") / entity.getMaxAttribute("health")) * 100
    );
  ...
  for (const target of promptee.combatants) {
    let currentPerc = Math.floor(
      (target.getAttribute("health") / target.getMaxAttribute("health")) * 100
    );
